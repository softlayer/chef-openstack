#Setup the softlayer neutron network.  Then, define the L3 services based on the neutron object ID's
action :setup_softlayer_networks do 
	...
	bash "Create neutron network for region" do
		environment "OS_USERNAME" => "admin",
					...
					"NEUTRON_PUB_SUBNET_NAME" => "sl-public-subnet",
					"NEUTRON_PUB_PHYS_NET" => "slpublic1",
					
					"NEUTRON_PRIV_NET_NAME" => "softlayer-private",
					"NEUTRON_PRIV_SUBNET_NAME" => "sl-private-subnet",
...
action :softlayer_l3_config do
	...
	command = Mixlib::ShellOut.new("neutron", "router-show", "public-router", "--variable", "id", "-f", "shell",
	...
	command = Mixlib::ShellOut.new("neutron", "router-show", "private-router", "--variable", "id", "-f", "shell",
	...
	command = Mixlib::ShellOut.new("neutron", "net-show", "softlayer-public", "--variable", "id", "-f", "shell",
	...
	command = Mixlib::ShellOut.new("neutron", "net-show", "softlayer-private", "--variable", "id", "-f", "shell",
	...
	service "neutron-l3-agent-public" do
		supports :status => true, :restart => true, :reload => true
		action [ :enable, :start ]
	end

	service "neutron-l3-agent-private" do
		supports :status => true, :restart => true, :reload => true
		action [ :enable, :start ]
	end

	%w[neutron-dhcp-agent neutron-plugin-openvswitch-agent neutron-metadata-agent neutron-lbaas-agent neutron-l3-agent-public neutron-l3-agent-private].each do |srv|
		service srv do
			action :restart
		end
	end
