...
node["keystone"]["default_accounts"]["users"].each do |user_name, info|
	grizzly_keystone "Create keystone user: #{user_name}" do 
		action :create_user
...
node["keystone"]["default_accounts"]["tenants"].each do |tenant_name|
	grizzly_keystone "Create keystone tenants: #{tenant_name}" do 
...
node["keystone"]["default_accounts"]["roles"].each do |role_name|
	grizzly_keystone "Create keystone role: #{role_name}" do 
		action :create_role
		role role_name
		keystone_service_pass node[:admin][:password]
	end
...
node["keystone"]["default_accounts"]["services"].each do |service, info|
	grizzly_keystone "Create keystone service: #{service}" do
...
node["keystone"]["default_accounts"]["user-roles"].each do |user_role|
	grizzly_keystone "Create keystone role: #{user_role}" do
...
grizzly_keystone "Create endpoint for nova" do
...
grizzly_keystone "Create endpoint for cinder" do
	action :create_endpoint
	region node.chef_environment
	keystone_service_pass node[:admin][:password]
...
grizzly_keystone "Create endpoint for glance" do
	action :create_endpoint
	region node.chef_environment
	keystone_service_pass node[:admin][:password]
...
grizzly_keystone "Create endpoint for keystone" do
	action :create_endpoint
	region node.chef_environment
	keystone_service_pass node[:admin][:password]
...
grizzly_keystone "Create endpoint for ec2" do
	action :create_endpoint
	region node.chef_environment
	keystone_service_pass node[:admin][:password]
