%W[cinder-api python-mysqldb cinder-scheduler cinder-volume python-cinderclient sysfsutils gdisk linux-headers-generic].each do |pkg|
	package pkg do
		action :install
	end
end
...
bash "Create #{node["cinder"]["config"]["volume_group"]} lvm partition" do
	...
	code <<-EOH
		...
		vgcreate #{node["cinder"]["config"]["volume_group"]} #{node["cinder"]["config"]["lvm_disk"]}
